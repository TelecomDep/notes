...
Обратя внимания на диаграмму можно заметить, что другие загрузчики классов написаны на ё (объекты `java.lang.ClassLoader`), что означает — их также необходимо загрузить в `JVM`! Эту задачу также выполняет `Bootstrap ClassLoader`.
...
`Preparation`: `JVM` **выделяет память** для статических переменных класса и инициализирует память значениями по умолчанию. 
