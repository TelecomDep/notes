...
3. Движение объектов (экземпляров) класса `Driver` должно быть **прямолинейным** (необходимо **перегрузить** функцию `move()` родительского класса);
...
6. При движении объектов **НЕ** должно происходить **коллизий** (2 объекта находятся на 1 точке пространства); Если возникла коллизия, меняем направление одного из объектов;
7. Отслеживанием коллизий должна заниматься функция `main()` посредством общего блока памяти для координатной плоскости и блокировка при помощи `mutex()`. 
...
1.1. Создать новое `Activity`, в котором будет реализованы функции `клиента` и `сервера`;
1.2. Реализовать начало передачу данных между сервером и клиентом при нажатии на кнопку (создать новую кнопку);
1.3. Новый функционал должен находится в отдельной (от основной) ветке (создаем новую ветку (`branch`));
2. Наладить передачу данных от Вашего приложения (Android) на приложение-сервер на Вашем компьютере (или VPS, если имеется):
2.2. `Сервер` (компьютер) - запустить серверную часть на базе языка `Python` + `ZMQ`;
2.3. `Android` - выступает в роли клиента (на базе `ZMQ`), подключается к серверу;
2.4. Ваш компьютер и телефон (`Android`) должны находится в одной сети (Wifi, USB, Internet (если есть белый `ip`-адрес));
2.5. В роли данных со стороны `Android`-приложения выступает строка `"Hello from Andorid!"`;
2.6. Данные со стороны сервера - `"Hello from Server!"`;
2.7. `Серверная` часть **ДОЛЖНА** сохранить (в файл) каждый блок данных, пришедший со смартфона и считать количество полученных пакетов.
2.8. Серверная часть также должны иметь метод\функцию по выводу на экран (консоль) всех сохраненных данных;
3. `Git`: 
3.1. `Android` - Сделать c merge request с новым функционалом;
3.2. `Server` - добавить новый коммит, обновить удаленный репозиторий.
