...
, где :math:`lat_{EPSG:4326}` - **latitude** в системе EPSG:4326, :math:`Y_{EPSG:3857}` - **X** в системе Меркатора.
...
1) Получаем значения координат тайла (в матрице всех тайлов для :math:`2^{zoom}`)
...
.. list-table:: Информация по тайлам для масштабов (0 - 19)
   ...
   * - Этап
     - Значения
   * - **Web-Меркатор**
   - :math:`X_{EPSG:3857} = 82.944546`; :math:`X_{EPSG:3857} = 1.1544770655`;
   * - **Нормирование**
   - :math:`x = 0.73040151666`; :math:`y = 0.31625926833`;
   * - **Масштаб**
   - :math:`2^{16} = 65536`;
   * - Точные индексы тайла по ``X``, ``Y``
   - :math:`x_{tile} = 0.73040151666 * 65536 = 47867.5937958`; :math:`y_{tile} = 0.31625926833 * 65536 = 20726.3674093`; :math:`fractional_{x} =59.37958` %; :math:`fractional_{y} =36.74093` %;
   * - Значения пикселей в картинке ``256x256``
     - X - :math:`256 * 0.5937 = 151.9872`;  Y - :math:`256 * 0.3674 = 94.0544`.

1) **Web-Меркатор**

:math:`X_{EPSG:3857} = 82.944546`;

:math:`X_{EPSG:3857} = 1.1544770655`;
...
:math:`fractional_{x}` и :math:`fractional_{y}` дадут нам значения пикселя для искомой точки координат:

5) Значения пикселей в картинке ``256x256``:

- Пиксель X - :math:`256 * 0.5937 = 151.9872`;
- Пиксель Y - :math:`256 * 0.3674 = 94.0544`.
