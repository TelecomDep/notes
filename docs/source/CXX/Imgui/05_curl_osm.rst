...
Список некоторых проекций можно посмотреть `здесь <https://ru.wikipedia.org/wiki/%D0%A1%D0%BF%D0%B8%D1%81%D0%BE%D0%BA_%D0%BA%D0%B0%D1%80%D1%82%D0%BE%D0%B3%D1%80%D0%B0%D1%84%D0%B8%D1%87%D0%B5%D1%81%D0%BA%D0%B8%D1%85_%D0%BF%D1%80%D0%BE%D0%B5%D0%BA%D1%86%D0%B8%D0%B9>`_.

``EPSG:4326`` — географическая система координат, основанная на системе параметров ``WGS84`` ((**World Geodetic System 1984**) традиционно использует порядок **широта** — **долгота**, а ``EPSG 4326`` —  **долгота** — **широта**.). Единица измерения — **градус**.
...
.. figure:: ./image/Worlds_animate.gif
   ...
   Авторство: Jakub Nowosad. Собственная работа, CC BY-SA 4.0, https://commons.wikimedia.org/w/index.php?curid=73955926 
...
1) Преобразование в Web-проекцию Меркатора
...
.. math::
  :label: (2)
   ...
   y_{EPSG:3857} = \ln(\tanh(lat_{EPSG:4326}) + \frac{1}{\cos(lat_{EPSG:4326})})
...
4) Получаем нумерацию тайлов (имя тайла в сетке для :math:`2^{zoom}`)
