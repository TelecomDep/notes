...
**Наша цель**: преобразовать GPS-координаты в позиции (``X``, ``Y``) тайлов Меркатора. 

Состоит из 4 этапов:

- Преобразовать углы ``Latitude``, ``Longitude`` в проекцию `Меркатора <https://ru.wikipedia.org/wiki/%D0%9F%D1%80%D0%BE%D0%B5%D0%BA%D1%86%D0%B8%D1%8F_%D0%9C%D0%B5%D1%80%D0%BA%D0%B0%D1%82%D0%BE%D1%80%D0%B0>`_;
...
А сейчас по формулам
^^^^^^^^^^^^^^^^^^^^^
1) Первое что нужно сделать - это посчитать количество тайлов, которое мы хотим отобразить на карте (см. :ref:`my-zoom-info-table`):
...
Пример преобразований Latitude и Longitude в проекцию Меркатора на языке ``СИ``:
