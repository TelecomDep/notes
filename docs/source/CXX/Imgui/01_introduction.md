...
Ориг. [ссыль](https://learn.microsoft.com/en-us/windows/win32/learnwin32/retained-mode-versus-immediate-mode) и [ссыль](https://ru.wikipedia.org/wiki/%D0%A1%D0%BE%D1%85%D1%80%D0%B0%D0%BD%D1%91%D0%BD%D0%BD%D1%8B%D0%B9_%D1%80%D0%B5%D0%B6%D0%B8%D0%BC).
...
![1770020903343](image/01_introduction/1770020903343.png)

**Пример Immediate Mode**
![1770020911045](image/01_introduction/1770020911045.png)
...
- `STATIC` - Статическая библиотека представляет собой собрание *.`obj`-файлов библиотеки (**НЕ** исполняемый файл), который мы можем, указав линкеру, прицепить к нашей программе в момент компиляции.  [Пример](image/imgui_introduction/imgui_cmake_static.png)

- `SHARED` - **УЖЕ** исполняемый файл. Динамическая линковка выполняется средствами платформы (операционной системы) в процессе работы программы. Все так же у нас в руках *.lib и *.h файлы, однако, теперь к ним добавляется `.dll` (`.so` для Linux). *.`lib`-файл теперь содержит **только вызовы** к `*.so`, где лежат непосредственно алгоритмы и которые вызываются уже на ходу, а не компилируются.[Пример](image/imgui_introduction/imgui_cmake_shared.png)
