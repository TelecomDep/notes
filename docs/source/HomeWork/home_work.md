...
2. Необходимо заргужать несколько тайлов (зависиот от текущего размера окна `ImGui`):
    ...
    2. Количество загружаемых тайлов должно динамически менять в зависимости от изменения размера окна.
3. Адаптировать вашу карту под изменения местоположения центра экрана в `zoom`'а:
    1. Т.е. имитировать перемещение карты "**на лету**".
4. Все уже загруженные тайлы сохранять в директории `build`:
    - Если тайл, который вы хотите отобразить уже был загружен, выводим на экран его (из файла)
